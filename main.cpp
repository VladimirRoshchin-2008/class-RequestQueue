...
public:
    ...
    vector<Document> AddFindRequest(const string& raw_query, DocumentPredicate document_predicate) {
        ...
        requests_.emplace_back(QueryResult{current_time_, results.empty()});
        if (results.empty()) {
            no_result_count_++;
        }
...
private:
    ...
    static const int MINUTES_IN_A_DAY = 1440;

    void RemoveOldRequests() {
        while (!requests_.empty() && requests_.front().time <= current_time_ - MINUTES_IN_A_DAY) {
